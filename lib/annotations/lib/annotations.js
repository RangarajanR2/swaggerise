...
var linetypes = {
  ...
  dbend   : { pattern: /^\s?\*\/$/ },
  middleware : {pattern: /^\s?[a-zA-Z]*\.use\(/ },
  ...
  fnsig10 : { pattern: /^\s?[a-zA-Z]*\.(post|get|put|patch|delete)\(([a-zA-Z0-9.+'":\/\s\[\]]+)\s*,/, returns: {meth:1,patt:2} },
  fnsig11 : { pattern: /^\s?module.exports\s*=\s*/, returns: { name:'module' } }
};
...
function identify(str) {
  ...
  if(str.trim() == ""){   //Skipping all empty lines.
    return false;
  }
...
function analyze(data) {
  ...
  for (var num in lines) {
    ...
    let line = lines[num];
    if(line.trim() != ""){    //Skipping empty lines
      // Get line type
    var type = identify(line);
    ...
    switch (type.id) {
      ...
      case 'fnsig10':
        // Store current annotations under function name
        annotations[routecounter] = annotations[routecounter] ? annotations[routecounter] : {};
        annotations[routecounter][type.meth] = current;
        current = {};
        routecounter++;
        break;
      case 'fnsig11':
        annotations['module'] = current;
        current = {};
        break;
      default: 
